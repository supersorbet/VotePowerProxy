...
contract CopeToken is BEP20("CopeToken", "COPE") {
...
contract WasterWeff is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    ///     COPE rewards each pool gets.
    /// `accCakePerShare` Accumulated COPEs per share, times 1e12.
    ...
    ///     the COPE rewards to all the WojakSwap products.
    ...
    /// @notice The only address can withdraw all the burn COPE.
    ...
    uint256 public constant MASTERCHEF_COPE_PER_BLOCK = 9375 * 1e14;
    uint256 public constant ACC_COPE_PRECISION = 1e18;
    ...
    uint256 public constant COPE_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of COPE burn action being executed.
    /// @notice COPE distribute % for burn
    uint256 public cakeRateToBurn = 0;
    /// @notice COPE distribute % for regular farm pool
    uint256 public cakeRateToRegularFarm = 250000000000;
    /// @notice COPE distribute % for special pools
    ...
    /// @param _cake The COPE token contract address.
    ...
    /// only for COPE distributions within WojakSwap products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake COPE token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != cake, "COPE token can't be added to farm pools");
    ...
    /// @notice Update the given pool's COPE allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending COPE rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    ...
    function pendingCake(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accCakePerShare = accCakePerShare.add(cakeReward.mul(ACC_COPE_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accCakePerShare).div(ACC_COPE_PRECISION).sub(user.rewardDebt);
    }
    ...
    /// @notice Calculates and returns the `amount` of COPE per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function cakePerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_COPE_PER_BLOCK.mul(cakeRateToRegularFarm).div(COPE_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_COPE_PER_BLOCK.mul(cakeRateToSpecialFarm).div(COPE_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of COPE per block to burn.
    function cakePerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_COPE_PER_BLOCK.mul(cakeRateToBurn).div(COPE_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accCakePerShare = pool.accCakePerShare.add((cakeReward.mul(ACC_COPE_PRECISION).div(lpSupply)));
    ...
    /// @notice Send COPE pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    ...
    /// @notice Update the % of COPE distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of COPE to burn each block.
    ...
    function updateCakeRate(
        ...
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == COPE_RATE_TOTAL_PRECISION,
    ...
    /// @notice Settles, distribute the pending COPE rewards for given user.
    ...
    function settlePendingCake(
        ...
        uint256 accCake = boostedAmount.mul(poolInfo[_pid].accCakePerShare).div(ACC_COPE_PRECISION);
    ...
    /// @notice Safe Transfer COPE.
    /// @param _to The COPE receiver address.
    /// @param _amount transfer COPE amounts.
